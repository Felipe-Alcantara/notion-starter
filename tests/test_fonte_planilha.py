...
def test_ingerir_adapta_titulo_e_campos_genericos_ao_schema(planilha_csv):
    class ClienteComSchema(ClienteFake):
        def __init__(self):
            super().__init__()
            self.consultas = 0
    ...
    assert props["Tarefa"]["title"][0]["text"]["content"] == "Conta A"
    assert props["Seguidores"] == {"number": 1614}
    # Colunas ausentes no destino não são enviadas nem usadas como filtro.
    assert "Nome" not in props and "Fonte" not in props and "Origem" not in props
...
def test_ingerir_sem_schema_mantem_comportamento_classico(planilha_csv):
    ...
    assert resultado.criados == 2
    props = cliente.criadas[0]
    assert "Nome" in props and "Fonte" in props and "Origem" in props
